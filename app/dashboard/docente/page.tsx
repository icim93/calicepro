...
export default async function DocenteHome() {
  ...
  return (
    <div>
      ...
      {prossima && (
        <>
          <p className="section-label" style={{ color: 'rgba(90,188,212,0.5)' }}>Prossima lezione</p>
          <Link href={`/dashboard/docente/lezioni/${prossima.id}`}
            ...
            <div className="flex justify-between items-start mb-2">
              ...
              <span className="text-[11px] text-cream/40">{prossima.ora_inizio?.slice(0,5)} – {prossima.ora_fine?.slice(0,5)}</span>
            </div>
            ...
            <div className="flex gap-2">
              <Link href={`/dashboard/docente/vini?lezione=${prossima.id}`}
              ...
              <Link href="/dashboard/docente/materiali"
                onClick={e => e.stopPropagation()}
      ...
      {/* CORSI ASSEGNATI */}
      <p className="section-label" style={{ color: 'rgba(90,188,212,0.5)' }}>Corsi assegnati</p>
      <div className="mx-4 bg-surf2 border border-white/[0.055] rounded-[18px] overflow-hidden mb-4">
        {(corsi ?? []).length === 0 ? (
          <p className="text-sm text-cream/40 text-center py-8">Nessun corso assegnato</p>
        ) : (corsi ?? []).map((corso, i) => (
          <Link key={corso.id} href={`/dashboard/docente/lezioni?corso=${corso.id}`}
            className={`flex items-center gap-3 p-4 hover:bg-white/[0.02] transition-colors
                        ${i < (corsi ?? []).length - 1 ? 'border-b border-white/[0.055]' : ''}`}>
            <div className="w-9 h-9 rounded-xl bg-doc/25 border border-doc-acc/20
      ...
      {lezErogate! > 0 && (
        <>
          <p className="section-label" style={{ color: 'rgba(90,188,212,0.5)' }}>Storico recente</p>
          <Link href="/dashboard/docente/calendario"
...
function ChevronRight() {
  return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" className="text-cream/20 flex-shrink-0"><polyline points="9 18 15 12 9 6"/></svg>
}
